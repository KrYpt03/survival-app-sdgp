...
New-Item -ItemType Directory -Path nginx\www -Force

# Check if OpenSSL is installed
try {
    $openssl = Get-Command openssl -ErrorAction Stop
    Write-Host "OpenSSL found at $($openssl.Source)"
} catch {
    Write-Host "OpenSSL not found. Please install OpenSSL and add it to your PATH." -ForegroundColor Red
...
DNS.1 = localhost
IP.1 = 127.0.0.1
"@ | Out-File -FilePath nginx\ssl\server.ext -Encoding ASCII
...
"@ | Out-File -FilePath nginx\www\index.html -Encoding UTF8
...
"@ | Out-File -FilePath nginx\www\404.html -Encoding UTF8
...
"@ | Out-File -FilePath nginx\www\50x.html -Encoding UTF8

Write-Host "SSL certificates and sample HTML files generated successfully." -ForegroundColor Green
Write-Host "Make sure to install the rootCA.crt certificate in your browser to trust the local SSL connection." 